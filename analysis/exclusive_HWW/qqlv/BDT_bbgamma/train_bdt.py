...
path = f"outputs/{args.energy}/preselection/qqlv/trainingdata_bba/"
sig_df = load_process(path + f"mgp8_ee_ha_ecm{args.energy}_hww.root", variables, weight_sf=weight_sf, target=1)
bkg = load_process(path + f"wzp6_ee_bba_ecm{args.energy}.root", variables, weight_sf=weight_sf)
...
print("Export model")
fOutName = f"outputs/{args.energy}/BDT/qqlv/bdt_model_bba_example.root"
...
pickle.dump(save, open(f"outputs/{args.energy}/BDT/qqlv/bdt_model_bba_example.pkl", "wb"))