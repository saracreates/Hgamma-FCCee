...
# Output directory
outputDir   = f"outputs/{ecm}/preselection/qqlv/trainingdata_bba/"
inputDir = "/afs/cern.ch/work/s/saaumill/public/analyses/symlink_gammalvqq" # path to the input data same as for lvqq
# WARNING these are for WW not bbgamma
...
if int(ecm) != 365: 
    raise ValueError("Only 365 GeV supported at the moment")
else:
    processList = {
        # cross sections given on the webpage: https://fcc-physics-events.web.cern.ch/fcc-ee/delphes/winter2023/idea/ 
        f'wzp6_ee_bba_ecm{ecm}': {'fraction': 1, 'crossSection': 0.985, 'inputDir': inputDir}, 
        f'mgp8_ee_ha_ecm{ecm}_hww':   {'fraction': 0.66, 'crossSection': xsec[str(ecm)][1], 'inputDir':inputDir}, # use TWO of the linked files for signal
    }
