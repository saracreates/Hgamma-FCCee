...
ecm = config['ecm']
input_bdt = os.path.join(config['outputDir'], str(ecm),'treemaker/BDT/', config_jj['outputDir_sub'], 'H{}{}'.format(args.flavor.lower(), args.flavor.lower()), 'bdt_model_example.pkl')
outDir = os.path.join(config['outputDir'], str(ecm),'plots/BDT/', config_jj['outputDir_sub'], 'H{}{}'.format(args.flavor.lower(), args.flavor.lower()))
