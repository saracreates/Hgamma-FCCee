...
hists["cutFlow"] = {
    ...
    "xtitle":   ["All events", f"iso < {config['cuts']['photon_iso_threshold']}", str(config['cuts']['photon_energy_range'][0]) + "< p_{#gamma} < " + str(config['cuts']['photon_energy_range'][1]), "|cos(#theta)_{#gamma}|<" + str(config['cuts']['photon_cos_theta_max']), f"n particles > {config['cuts']['min_n_reco_no_gamma']}", str(recoil_mass_min) + " < m_{recoil} < " + str(recoil_mass_max), str(m_jj_min) + "< m_{jj} < " + str(m_jj_max), str(sum_jetscores) + " < sum jet scores", str(signal_mass_min) + " < m_{recoil} < " + str(signal_mass_max)],
    "ytitle":   "Events ",
}
